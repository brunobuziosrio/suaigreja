create table if not exists public.festa_order_items (
  id uuid primary key default gen_random_uuid(),
  order_id uuid not null references public.festa_orders(id) on delete cascade,
  product_id uuid references public.festa_products(id) on delete set null,
...
create index if not exists idx_festa_order_items_order on public.festa_order_items(order_id);
alter table public.festa_order_items enable row level security;
create policy "account members manage festa order items" on public.festa_order_items for all using (exists (select 1 from public.festa_orders o join public.festa_events e on e.id = o.festa_event_id where o.id = order_id and public.is_account_member(e.account_id, auth.uid()))) with check (exists (select 1 from public.festa_orders o join public.festa_events e on e.id = o.festa_event_id where o.id = order_id and public.is_account_member(e.account_id, auth.uid())));
...
begin
  ...
  select festa_event_id, e.account_id into v_event_id, v_account_id from festa_stalls s join festa_events e on e.id=s.festa_event_id where s.id=p_stall_id and s.active;
  if v_event_id is null or not public.is_account_member(v_account_id, auth.uid()) then raise exception 'Barraca indisponível'; end if;
...
grant execute on function public.record_festa_sale(uuid,text,jsonb) to authenticated;
