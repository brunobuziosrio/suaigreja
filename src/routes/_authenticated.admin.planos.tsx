...
import { listPlanFeatureFlags, updatePlanFeatureFlag } from "@/lib/admin.functions";
...
function PlanFeaturesPage() {
  ...
  return <AppShell><div className="mx-auto w-full max-w-6xl space-y-6">
    <header className="rounded-2xl border bg-gradient-to-br from-slate-950 to-slate-800 p-6 text-white shadow-lg">
      <div className="flex items-start gap-4"><span className="rounded-xl bg-white/10 p-3"><LockKeyhole className="h-6 w-6" /></span><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-200">Catálogo comercial</p><h1 className="mt-1 text-2xl font-semibold">Recursos por plano</h1><p className="mt-2 max-w-2xl text-sm text-slate-300">Ative ou desative cada recurso para cada plano. As mudanças afetam novas verificações de acesso após a migration.</p></div></div>
    </header>
    <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead><tr className="border-b bg-muted/40"><th className="p-4 text-left">Recurso</th>{tiers.map((tier) => <th key={tier.id} className="p-4 text-center">{tier.label}</th>)}</tr></thead><tbody>{MODULE_CATALOG.map((feature) => <tr key={feature.id} className="border-b last:border-0"><td className="p-4"><div className="font-medium">{feature.label}</div><div className="mt-1 flex gap-2"><Badge variant="outline">{feature.status}</Badge><span className="text-xs text-muted-foreground">{feature.path}</span></div></td>{tiers.map((tier) => <td key={tier.id} className={`p-4 text-center ${tier.tone}`}><Switch checked={enabled(feature.id, tier.id)} disabled={isLoading || mutation.isPending} onCheckedChange={(value) => mutation.mutate({ feature_id: feature.id, plan_tier: tier.id, enabled: value })} aria-label={`${feature.label} no plano ${tier.label}`} /></td>)}</tr>)}</tbody></table></div></Card>
